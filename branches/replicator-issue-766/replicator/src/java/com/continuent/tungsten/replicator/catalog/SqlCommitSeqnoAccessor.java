...
public class SqlCommitSeqnoAccessor implements CommitSeqnoAccessor
{
    ...
    public void prepare() throws ReplicatorException
    {
        ...
        try
        {
            ...
            commitSeqnoUpdate = conn.prepareStatement("UPDATE "
                    + commitSeqnoTable.getSchema() + "."
                    + commitSeqnoTable.getName() + " SET seqno=?, "
                    + "fragno=?, " + "last_frag=?, " + "source_id=?, "
                    + "epoch_number=?, " + "eventid=?, "
                    + "applied_latency=?, " + "update_timestamp=?, "
                    + "shard_id=?, " + "extract_timestamp=? "
                    + "WHERE task_id=?");
        }
